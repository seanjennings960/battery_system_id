...
class Segment:
    ...
    t1: float
    mode: str
    observation_times: FloatArray = field(default_factory=lambda: np.empty(0))
    observations: FloatArray = field(default_factory=lambda: np.empty((0, 0)))
    observation_sigma: FloatArray = field(default_factory=lambda: np.empty((0, 0)))
    ...
    def __post_init__(self) -> None:
        ...
        if times.shape[0] != values.shape[0] or values.shape != sigma.shape:
            raise ValueError("Observation times, values, and sigma have incompatible shapes")
    ...
    def empty(
        ...
        *,
        ...
        return cls(
            ...
            t1=t1,
            mode=mode,
            observation_times=np.empty(0),
            observations=np.empty((0, output_dim)),
            observation_sigma=np.empty((0, output_dim)),
...
class HybridODEModel:
    ...
    def physical_parameters(self, eta: ArrayLike) -> FloatArray:
        ...
        if eta_array.shape != (self.parameter_dim,):
            raise ValueError(f"eta must have shape {(self.parameter_dim,)}")
        theta = np.asarray(self.parameter_transform(eta_array), dtype=float)
        if theta.shape != (self.parameter_dim,) or not np.all(np.isfinite(theta)):
            raise ValueError("parameter_transform returned invalid parameters")
        return theta
    ...
    def flow(
        ...
        if times.size == 0:
            return np.empty((0, self.state_dim))
        ...
        if self.custom_flow is not None:
            result = np.asarray(self.custom_flow(segment, state0, parameters, times))
            if result.shape != (times.size, self.state_dim):
                raise ValueError("custom_flow returned the wrong shape")
            return result
        ...
        propagated = np.empty((0, self.state_dim))
        if solve_times.size:
            solution = solve_ivp(
                lambda t, x: np.asarray(self.rhs(t, x, parameters, segment)),
        ...
        unique_states = np.vstack([state0, propagated]) if includes_t0 else propagated
    ...
    def output(
        ...
        value = np.asarray(
            self.observe(float(time), np.asarray(state), np.asarray(theta), segment),
            dtype=float,
        )
        if value.ndim != 1:
